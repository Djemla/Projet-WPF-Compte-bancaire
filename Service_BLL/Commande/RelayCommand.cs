...
namespace Service_BLL.Commande
{
    // J'ai préféré crée le dossier et la classe commande ici dans la bilio BLL Service, car BLL a besoin de cette classe commande pour definir les boutons pour l'ouverture des fenêtres.
    // // Or si le dossier est dans le projet, BLL Service n'aura pas accès (vu qu'il ne peut pas être lié au projet par reference.
    // Par contre le projet est lié à BLL Service par référence, et peut donc avoir accès au dossier Commande

    public class RelayCommand : ICommand // du systeme: Icommande est l'interface qui fournit les evenements liés aux bouttons. 
